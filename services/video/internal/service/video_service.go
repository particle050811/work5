...
func (s *VideoService) ensureHotVideosCache(ctx context.Context, redisCli repository.RedisClient) (bool, bool, error) {
...
func (s *VideoService) rebuildHotVideosCache(ctx context.Context, redisCli repository.RedisClient) (bool, error) {
	var top []model.Video
	if err := s.store.DB().
		Model(&model.Video{}).
		Order(hotScoreOrderSQL()).
