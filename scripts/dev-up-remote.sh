...
ensure_port_available() {
  ...
  if port_in_use "$port"; then
    echo "[dev-up-remote] 端口已被占用 name=$name port=$port" >&2
    exit 1
  fi
}
...
ETCD_HOST_PORT="${ETCD_HOST_PORT:-32379}"
GATEWAY_HTTP_PORT="${GATEWAY_HTTP_PORT:-28888}"
CHAT_HTTP_PORT="${CHAT_HTTP_PORT:-28889}"
