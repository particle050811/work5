...
ENV_FILE="$RUNTIME_DIR/micro.remote.env"
COMPOSE_FILE="$ROOT_DIR/deploy/docker-compose.remote.yml"
...
choose_port() {
  ...
  echo "[dev-up-remote] 无法找到可用端口 preferred=$preferred fallback=$fallback" >&2
  exit 1
}
...
wait_for_health() {
  ...
  echo "[dev-up-remote] 等待容器健康状态超时 service=$service container=${container:-unknown} status=${status:-unknown}" >&2
  return 1
}
...
wait_for_http() {
  ...
  echo "[dev-up-remote] 等待 HTTP 服务超时 name=$name url=$url" >&2
  return 1
}
...
ETCD_HOST_PORT="${ETCD_HOST_PORT:-$(choose_port 2379 32379)}"
reserve_port "$ETCD_HOST_PORT"
MYSQL_HOST_PORT="${MYSQL_HOST_PORT:-$(choose_port 3306 33306)}"
reserve_port "$MYSQL_HOST_PORT"
REDIS_HOST_PORT="${REDIS_HOST_PORT:-$(choose_port 6379 36379)}"
reserve_port "$REDIS_HOST_PORT"
GATEWAY_HTTP_PORT="${GATEWAY_HTTP_PORT:-$(choose_port 8888 28888)}"
reserve_port "$GATEWAY_HTTP_PORT"
CHAT_HTTP_PORT="${CHAT_HTTP_PORT:-$(choose_port 8889 28889)}"
reserve_port "$CHAT_HTTP_PORT"

export ETCD_HOST_PORT MYSQL_HOST_PORT REDIS_HOST_PORT
...
echo "[dev-up-remote] gateway 地址: $BASE_URL"
echo "[dev-up-remote] chat-service WebSocket 地址: $CHAT_BASE_URL"
echo "[dev-up-remote] 运行环境文件: $ENV_FILE"
